...
import com.tmc.safetynet.Model.FirRecord;
...
public class ReportFIR extends AppCompatActivity {
    private final FirebaseDatabase firDb = FirebaseDatabase.getInstance();
    private final DatabaseReference firRef = firDb.getReference().child("FIRdetail");
...
//                });
        ...
        firRef.child(Token).updateChildren(firDetail)
                ...
                .addOnFailureListener(new OnFailureListener() {
                    @Override
                    public void onFailure(@NonNull Exception e) {
                        Toast.makeText(ReportFIR.this, "Failed" + e, Toast.LENGTH_SHORT).show();
                    }
                });
